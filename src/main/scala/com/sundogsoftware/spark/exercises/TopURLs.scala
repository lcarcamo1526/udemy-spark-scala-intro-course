...
object TopURLs {
  ...
  def main(args: Array[String]) {
    ...
    val statusCountsDF = logsDF2
      .groupBy(window(col("eventTime"), windowDuration = "30 seconds", slideDuration = "10 seconds"),
                  col("endpoint")
      ).count()
      .orderBy(col("count").desc)

    // Display the stream to the console
    val query = statusCountsDF.writeStream.outputMode("update")
